package com.fastclient.management.profile;
...
import com.fastclient.management.config.Config;
...
public class ProfileManager {
	...
	public void save(String name, String author, String serverIp, Object icon, ConfigType... types) {
		...
		ConfigManager configManager = Fast.getInstance().getConfigManager();
	...
	public void load(Profile profile) {
		...
		for (ObjectObjectImmutablePair<ConfigType, JsonObject> p : configs) {
			...
			Config config = Fast.getInstance().getConfigManager().getConfig(p.left());
					...
					while ((entry = zipIn.getNextEntry()) != null) {
						if (!entry.isDirectory()) {
							if (entry.getName().endsWith(".json")) {
								...
								if (type != null) {
									configs.add(ObjectObjectImmutablePair.of(type, readJsonFromZip(zipIn)));
								}
