...
import com.fastclient.utils.RandomUtils;
	...
	public void load() throws Exception {
		...
		for (File f : musicDir.listFiles()) {
			...
			if (name.endsWith(".flac")) {
				...
				if (title == null) title = name.replace(".flac", "");
				if (artist == null) artist = "";

			} else if (name.endsWith(".mp3")) {
				com.fastclient.utils.ID3Utils.Metadata meta = com.fastclient.utils.ID3Utils.extractMetadata(f);
			...
			musics.add(new Music(f, title, artist, album.exists() ? album : null, color));
		}
	}
