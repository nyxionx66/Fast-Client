package com.fastclient.management.hypixel;
...
import com.fastclient.management.websocket.packet.impl.SC_HypixelStatsPacket;
		...
		if (ServerUtils.isJoin(Server.HYPIXEL)) {
			if (timer.delay(500)) {
				...
				if (iterator.hasNext()) {
					...
					Fast.getInstance().getWebSocketManager().send(new SC_HypixelStatsPacket(request));
