...
- `IMixinCameraEntity`: Exposes independent cam pitch/yaw.
...
### Duck Typing (Interface Injection)
To expose Mixin-added functionality to the rest of the client, we use "Duck Interfaces" located in `com.fastclient.mixin.interfaces`. 
- **Example**: `IMixinMinecraftClient` allows other classes to access the `assetDir` which is captured during client initialization.

### Injection Points (`At`)
- `HEAD`: Code runs at the very beginning of the target method.
- `TAIL`: Code runs just before the method returns.
- `INVOKE`: Code is injected before/after a specific method call within the target.
- `RETURN`: Specifically targets return statements, often used with `CallbackInfoReturnable`.

---

> [!NOTE]
> All mixins are located in `src/main/java/com/fastclient/mixin/mixins`.
